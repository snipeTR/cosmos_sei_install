...
sudo wget https://raw.githubusercontent.com/snipeTR/sei_help/main/stafi_help.sh && chmod +x ./stafi_help.sh &&sudo mv ./stafi_help.sh /usr/local/bin/helpstafi
sudo wget https://raw.githubusercontent.com/snipeTR/sei_help/main/helpstafihubupdate && chmod +x ./helpstafihubupdate &&sudo mv ./helpstafihubupdate /usr/local/bin/helpstafihubupdate
...
echo '=============== SETUP FINISHED ==================='
echo -e "\n\e[1m\e[32mHere are some \e[0mCOMANDS\e[1m\e[32m that will make your validator job easier.\n\n\e[1m\e[32mhelpstafi\n\e[1m\e[34mIt gives information about some commands about stafihub validator.\nstafihubd service or application must be running.\n\n\e[1m\e[32mhelpstafihubupdate\n\e[1m\e[34mhelpstafi updates the command.\n\n\e[1m\e[32mstafihubport\n\e[1m\e[34mLists all ports used by stafihubd.\ndetailed information in $HOME/.stafihub/config/config.toml\e[0m\n\n\n"
sleep 2
   ...
   read -rsn1 answer
    if [ "$answer" != "${answer#[Yy]}" ] ;then
         ...
         if [ "$aanswer" != "${aanswer#[Yy]}" ] ;then
         	  if [[ $(stafihubd keys list --output json | jq .[0].name) == "\"$WALLET"\" || $(stafihubd keys list --output json | jq .[1].name) == "\"$WALLET"\" || $(stafihubd keys list --output json | jq .[2].name) == "\"$WALLET"\" || $(stafihubd keys list --output json | jq .[3].name) == "\"$WALLET"\" ]]; then 
         		  ...
         		  echo "export WALLET=$WALLETT" >> "$HOME"/.bash_profile
...
echo -e "\e[0m\e[01mIf you want to run stafihub public-testnet-v3 NODE with linux services. Run the script \e[0m\e[36m"stafihubd_start_with_service.sh".\e[0m"
echo "----------------------------------------------------"
